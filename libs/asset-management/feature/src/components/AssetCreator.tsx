...
import { Button, Dialog, DialogContent, Spinner } from '@micra-pro/shared/ui';
...
export const AssetCreator: Component<{
  ...
  const onCreate = (id: string) => props.onCreate(id);
  const close = () => props.onClose();
  const confirm = () => {
    setQr(undefined);
    setTimeout(() => accessor.refetch().finally(close), 1500);
  };
  return (
    <Dialog
      open={props.isOpen}
      ...
      >
        ...
        <Show when={qr()}>
          <div class="flex h-full w-full flex-col items-center justify-center">
            ...
            <Button variant="default" class="w-24" onClick={confirm}>
              Done
            </Button>
          </div>
        </Show>
