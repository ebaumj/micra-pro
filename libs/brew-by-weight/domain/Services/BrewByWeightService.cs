...
) : IBrewByWeightService
{
        ...
        var subscription = Observable
            .FromAsync(async ct =>
            {
                try
                {
                    ...
                    subject.OnNext(
                        new BrewByWeightTracking.Finished(result.Flow, result.Weight, result.Time)
                    );
                }
    ...
    private async Task RunBrewByWeightAsync(
        Guid beanId,
        Guid scaleId,
        double inCupQuantity,
        double grindSetting,
    ...
    )
    {
        ...
        await connection
            .Data.TakeUntil(d => d.Weight is < 0.2 and > -0.2)
            .Select(_ => Observable.FromAsync(cn => connection.TareAsync(cn)))
            .ToTask(ct);
        ...
        var finishTime = stopwatch.Elapsed + TimeSpan.FromSeconds(15);
        await connection
